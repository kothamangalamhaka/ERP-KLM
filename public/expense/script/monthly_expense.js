...
async function handleEntrySave() {
  ...
  if (!entry.particulars || entry.amount <= 0)
    return showToast("Check Details!", "#ef4444");
...
function renderTable() {
  ...
  dataToRender.forEach((e) => {
    ...
    if (e.category === "Office Account" || e.category === "Opening Balance")
      badgeClass = "bg-green";
    else if (e.category.includes("Settlement")) badgeClass = "bg-blue";
    else if (
      e.category === "Expense" ||
      e.category === "Office Rent" ||
      e.category === "Office Electricity"
    )
      badgeClass = "bg-red";
    // 🟢 Fixed Assets badge logic
    else if (e.category === "Fixed Assets") badgeClass = "bg-purple";

    let catBadge = `<span class="badge ${badgeClass}">${e.category}</span>`;
    let escapedEntry = JSON.stringify(e)
      .replace(/'/g, "&apos;")
      .replace(/"/g, "&quot;");
...
function calculateDashboard(data) {
  ...
  data.forEach((e) => {
    ...
    // 1. Calculate Expenses & Asset Breakdown
    if (e.category === "Office Rent") rentTotal += amt;
    else if (e.category === "Office Electricity") electTotal += amt;
    else if (e.category === "Expense" || e.category === "Purchase")
      generalTotal += amt;
    else if (e.category === "Fixed Assets") assetTotal += amt; // 🟢 Added Fixed Assets to total
    ...
    if (e.category === "Opening Balance" || e.category === "Office Account") {
      ...
      e.category === "Office Rent" ||
      e.category === "Office Electricity" ||
      e.category === "Fixed Assets" // 🟢 Deduct from office balance if fixed asset
    ) {
  ...
  let totalExp = rentTotal + electTotal + generalTotal + assetTotal; // 🟢 Total expenses including assets
  ...
  document.getElementById("totalStrip").innerHTML = `
        <div class="total-strip">
            ...
            <div class="strip-breakdown">
                ...
                <span>Fixed Assets: ₹ ${assetTotal.toLocaleString("en-IN")}</span> 
            </div>
        </div>`;
}
...
function openFilterMenu(e, col) {
  ...
  const from = document.getElementById("filterFrom").value;
  const to = document.getElementById("filterTo").value;
  let baseData = journalEntries.filter(
  ...
  const visibleOptionsData = baseData.filter((row) => {
    for (let c in activeTableFilters) {
      if (c === col) continue;
      if (
        activeTableFilters[c].length > 0 &&
        !activeTableFilters[c].includes(getFormattedValue(row, c))
      )
        return false;
    }
    return true;
  ...
  const uniqueValues = [
    ...new Set(visibleOptionsData.map((row) => getFormattedValue(row, col))),
  ].sort();

  optionsContainer.innerHTML = "";
  uniqueValues.forEach((val) => {
    const isChecked =
      activeTableFilters[col] && activeTableFilters[col].includes(val);
    optionsContainer.innerHTML += `
...
function downloadImage() {
  ...
  html2canvas(document.getElementById("ledgerContent"), { scale: 2 }).then(
    (canvas) => {
      const link = document.createElement("a");
      link.download = "KLM_Report.png";
...
// 🟢 Excel Export modified to use fetch to send authorization headers
async function exportToExcel() {
  const token = sessionStorage.getItem("expense_token");
  const from = document.getElementById("filterFrom").value;
  const to = document.getElementById("filterTo").value;

  let url = `/expenses/export-excel?`;
  let params = [];
  ...
  try {
    ...
    a.href = downloadUrl;
    a.download = "KLM_Expense_Report.xlsx";
    document.body.appendChild(a);
    a.click();
    window.URL.revokeObjectURL(downloadUrl);
    a.remove();

    showToast("Excel Downloaded Successfully!", "#10b981");
  } catch (error) {
    showToast(
      "Export failed. Check your connection or login again.",
      "#ef4444",
    );
  } finally {

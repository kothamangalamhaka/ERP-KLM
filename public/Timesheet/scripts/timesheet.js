document.addEventListener("DOMContentLoaded", () => {
    ...
    userRole = user.role;
    document.getElementById('userInfo').innerText = `👤 ${user.username} (${userRole})`;

    const hour = new Date().getHours();
    let greeting = "Good evening";
    if (hour < 12) greeting = "Good morning";
    else if (hour < 18) greeting = "Good afternoon";
    document.getElementById('welcomeText').innerText = `${greeting}, ${user.username}!`;
...
async function fetchRules() {
    try {
        const token = localStorage.getItem('timesheetToken');
        const res = await fetch('/timesheet/api/rules', { headers: { 'Authorization': 'Bearer ' + token } });
...
async function addRule() {
    try {
        ...
        if (!site) { customAlert('Warning', 'Please enter a Site Keyword.'); return; }
...
async function exportExcel() {
    ...
    try {
        ...
        let ws_data = [
            ["Plate No", "Date", "Wrk Start", "Wrk end", "HMR Start", "HMR End", "Fuel", "BD", "Remark", "NL", "Distance", "Time"]
        ];

        if (data.success && data.data && data.data.length > 0) {
            data.data.forEach(row => {
                ws_data.push([
                    row.plate_no, row.record_date, row.wrk_start || "", row.wrk_end || "",
                    row.hmr_start || "", row.hmr_end || "", row.fuel || "", row.bd || "",
                    row.remark || "", row.nl_checked ? "TRUE" : "FALSE", row.calc_distance || "", row.calc_time || ""
                ]);
            });
        } else {
...
async function importExcel() {
    ...
    try {
        ...
        function calcRowDistTime(row, site) {
            let hs = parseFloat(row["HMR Start"]); let he = parseFloat(row["HMR End"]);
            let dist = null; if (!isNaN(hs) && !isNaN(he)) dist = (he - hs).toFixed(2);

            let finalTime = null;
            let ws = String(row["Wrk Start"] || "").trim(); let we = String(row["Wrk end"] || "").trim();
            let bd = String(row["BD"] || "").trim().toUpperCase();
            ...
            if (bd) {
                ...
                let parseRT = (val) => {
                    let [hStr, mStr] = String(val).split('.'); let h = parseInt(hStr) || 0;
                    let m = 0; if (mStr) { mStr = mStr.length === 1 ? mStr + '0' : mStr.substring(0, 2); m = parseInt(mStr); }
                    return h + (m / 60);
                };
        ...
        reader.onload = async function (e) {
            ...
            jsonData.forEach(row => {
                ...
                processedRecords.push({
                    month: m, year: y,
                    plate_no: pNo, record_date: rDate.toString(),
            ...
            if (processedRecords.length === 0) {
                ...
                sts.style.color = "#842029"; sts.style.backgroundColor = "#f8d7da";
                return;
            }
            ...
            try {
                ...
                if (sendRes.ok && resData.success) {
                    ...
                    sts.style.color = "#0f5132"; sts.style.backgroundColor = "#d1e7dd";

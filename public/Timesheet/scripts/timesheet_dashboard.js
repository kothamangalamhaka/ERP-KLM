...
function openFilterMenu(event, key, colIndex) {
    ...
    for (let i = 0; i < trs.length; i++) {
        ...
        for (let filterKey in activeFilters) {
            ...
            let cIdx = filterKey === 'owner' ? 1 : filterKey === 'type' ? 2 : filterKey === 'site' ? 5 : 6;
...
function getGapStatus(d, sLogs, dLogs) {
    ...
    if (sLogs && sLogs.length > 0) {
        ...
        for (let i = 0; i < ascSLogs.length; i++) {
            ...
            if (d > ed) { if (ascSLogs[i].status === 'Replaced') isReplaced = true; }
        }
        if (!sActive) {
    ...
    if (dLogs && dLogs.length > 0) {
        ...
        if (!dActive) {
            let firstStart = parseLogDate(ascDLogs[0].work_start_date, new Date('2000-01-01'));
            let lastEnd = parseLogDate(ascDLogs[ascDLogs.length - 1].work_end_date, new Date('2099-01-01'));
            if (d >= firstStart && d <= lastEnd) dGap = true;
        }
    } else { dActive = true; }
    if (!dActive) return dGap ? "DC" : "AB";
    return "ACTIVE";
}
...
function applyGridFilters(tableId = 'dashboardTable') {
    ...
    const fDriver = document.getElementById('fDriver') ? document.getElementById('fDriver').value.toUpperCase() : "";
    ...
    for (let i = 0; i < trs.length; i++) {
        ...
        if (activeFilters.type.length > 0 && !activeFilters.type.includes(tType)) matchesFilters = false;
...
function toggleEditMode() {
    ...
    container.innerHTML = getTableHTMLString(cachedDashboardData, document.getElementById('selMonth').value, document.getElementById('selYear').value, isEditMode);
...
function recalculateRowOnEdit(cell) {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        targetCell.className = targetCell.className.replace(/code-[a-z]+|cell-fri/g, '').trim();
        if (isFullOT) targetCell.classList.add('cell-fri');

        if (cellVal === 'B') { targetCell.classList.add('code-b'); }
        else if (cellVal === 'H') { targetCell.classList.add('code-h'); }
        else if (['ID', 'NP'].includes(cellVal)) {
            targetCell.classList.add(cellVal === 'ID' ? 'code-id' : 'code-np');
            if (isFullOT) { normalHr = 0; otHr = 10; } else { normalHr = 10; otHr = 0; }
        }
        else if (['AB'].includes(cellVal)) { targetCell.classList.add('code-ab'); }
        else if (['R'].includes(cellVal)) { targetCell.classList.add('code-r'); }
        else if (['DC'].includes(cellVal)) { targetCell.classList.add('code-dc'); }
        else if (['SC'].includes(cellVal)) { targetCell.classList.add('code-sc'); }
        else if (!isNaN(timeRaw) && timeRaw > 0) {
            if (isFullOT) { otHr = timeRaw; }
            else { if (timeRaw > 10) { normalHr = 10; otHr = timeRaw - 10; } else { normalHr = timeRaw; } }
        }

        sumNormal += normalHr; sumOT += otHr;
    }
...
async function fetchAndGenerateDashboard() {
    ...
    container.innerHTML = '<div style="color: #64748b; margin-top: 50px; text-align:center;">Fetching and processing data...</div>';
    ...
    try {
        ...
        if (data.vehicles.length === 0) {
            container.innerHTML = '<div style="color: #b91c1c; margin-top: 50px; text-align:center; font-weight:bold;">No records found.</div>';
            return;
        }
...
function getTableHTMLString(data, m, y, isEditMode, tableId = 'dashboardTable') {
    const daysInMonth = getDaysInMonth(m, y);

    const ownerFilterClass = activeFilters.owner.length > 0 ? 'filter-active' : '';
    const ownerFilterIcon = activeFilters.owner.length > 0 ? 'filter_alt' : 'filter_list';

    const typeFilterClass = activeFilters.type.length > 0 ? 'filter-active' : '';
    const typeFilterIcon = activeFilters.type.length > 0 ? 'filter_alt' : 'filter_list';

    const siteFilterClass = activeFilters.site.length > 0 ? 'filter-active' : '';
    const siteFilterIcon = activeFilters.site.length > 0 ? 'filter_alt' : 'filter_list';

    const plateFilterClass = activeFilters.plate.length > 0 ? 'filter-active' : '';
    const plateFilterIcon = activeFilters.plate.length > 0 ? 'filter_alt' : 'filter_list';
...
`;
    ...
    let processedVehicles = data.vehicles.map(v => {
        ...
        v.currDriver = activeDrivers.map(d => d.driver_name).join(' & ') || (v.driver_name || "");
        v.currMobile = activeDrivers.map(d => d.driver_mobile).join(' & ') || (v.driver_mobile || "");
        v.currSite = activeSites.map(s => s.site_name).join(' & ') || (v.site_name || "");
    ...
    `;
        ...
        let sumNormal = 0, sumOT = 0, sumDist = 0, sumFuel = 0;
        let editProps = isEditMode ? 'contenteditable="true" class="editable-cell" onblur="recalculateRowOnEdit(this)"' : '';
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            let hasData = timeRaw > 0 || bdStr !== '' || rec.wrk_start || rec.hmr_start;

            if (hasData) {
                if (['B', 'H', 'ID', 'NP'].includes(bdStr)) cellDisplay = bdStr;
                else cellDisplay = timeRaw > 0 ? timeRaw : '';

                if (bdStr === 'B') { cellClass += " code-b"; }
                else if (bdStr === 'H') { cellClass += " code-h"; }
            ...
            sumNormal += normalHr; sumOT += otHr; sumDist += dist; sumFuel += fuel;
...
function buildWorksheetFromTable(table, m, y) {
    ...
    for (let i = 0; i < visibleTrs.length; i++) { ws['!rows'].push({ hpt: 26 }); }
    ...
    for (let R = 0; R < totalRows; ++R) {
        for (let C = 0; C < totalCols; ++C) {
            ...
            if (R === 0) {
                ...
                cellStyle.font = { name: "Arial", sz: 14, color: { rgb: "FFFFFF" }, bold: true };
            } else if (R === 1 || R === 2) {
                cellStyle.fill = { fgColor: { rgb: "1e3a5f" } };
                cellStyle.font = { name: "Arial", sz: 10, color: { rgb: "FFFFFF" }, bold: true };
                if (R === 1 && C >= 7 && C < 7 + daysInMonth) {
                    cellStyle.alignment = { horizontal: "center", vertical: "center", textRotation: 90 };
                }
            } else {
                let val = String(ws[cellRef].v).trim();
                if (C === 1 || C === 3 || C === 5) cellStyle.alignment.horizontal = "left";

                if (C === totalCols - 5) cellStyle.fill = { fgColor: { rgb: "78a664" } };
                else if (C === totalCols - 4) cellStyle.fill = { fgColor: { rgb: "9fc5e8" } };
                else if (C === totalCols - 3) cellStyle.fill = { fgColor: { rgb: "45818e" } };
                else if (C === totalCols - 2) cellStyle.fill = { fgColor: { rgb: "a4c2f4" } };
                else if (C === totalCols - 1) cellStyle.fill = { fgColor: { rgb: "75b4d8" } };
                    ...
                    if (val === 'B') {
                        ...
                        cellStyle.font = { name: "Arial", sz: 10, color: { rgb: "854d0e" }, bold: true };
                    } else if (val === 'NP') {
                        cellStyle.fill = { fgColor: { rgb: "86efac" } };
                        cellStyle.font = { name: "Arial", sz: 10, color: { rgb: "14532d" }, bold: true };
                    } else if (val === 'AB') {
    ...
    let colWidths = [{ wch: 5 }, { wch: 25 }, { wch: 15 }, { wch: 25 }, { wch: 14 }, { wch: 20 }, { wch: 12 }];
    for (let i = 0; i < daysInMonth; i++) colWidths.push({ wch: 4.2 });
    colWidths.push({ wch: 10 }, { wch: 10 }, { wch: 12 }, { wch: 10 }, { wch: 12 });
    ws['!cols'] = colWidths;
...
function exportCurrentExcel() {
    const table = document.getElementById("dashboardTable");
    if (!table) { alert("Please fetch data first."); return; }
    ...
    try {
        ...
        while (current <= endDate) {
            ...
            const res = await fetch('/timesheet/api/public/view-report', {
                ...
                body: JSON.stringify({ month: curM, year: curY, filterType: 'All', filterValue: '' })
            });
            const data = await res.json();

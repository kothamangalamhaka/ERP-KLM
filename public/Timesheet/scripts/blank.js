...
// Filtering Variables
let activeFilters = { owner: [], driver: [], site: [], vehicleType: [], plate: [], pendingDates: [] };
...
function openFilterMenu(event, key, colIndex) {
    ...
    for (let i = 0; i < trs.length; i++) {
        ...
        for (let filterKey in activeFilters) {
            ...
            let cIdx = filterKey === 'owner' ? 1 : filterKey === 'driver' ? 3 : filterKey === 'site' ? 5 : filterKey === 'vehicleType' ? 6 : filterKey === 'plate' ? 7 : 8; let cellVal = tds[cIdx].textContent.trim();
...
function applyGridFilters() {
    ...
    for (let i = 0; i < trs.length; i++) {
        ...
        if (activeFilters.vehicleType.length > 0 && !activeFilters.vehicleType.includes(tVehicleType)) matchesFilters = false;
        if (activeFilters.plate.length > 0 && !activeFilters.plate.includes(tPlate)) matchesFilters = false;
        if (activeFilters.pendingDates.length > 0 && !activeFilters.pendingDates.includes(tPending)) matchesFilters = false;
...
async function generatePendingReport() {
    ...
    try {
        ...
        activeFilters = { owner: [], driver: [], site: [], vehicleType: [], plate: [], pendingDates: [] };
        ...
        const vtFilterClass = activeFilters.vehicleType && activeFilters.vehicleType.length > 0 ? 'filter-active' : '';
        const vtFilterIcon = activeFilters.vehicleType && activeFilters.vehicleType.length > 0 ? 'filter_alt' : 'filter_list';
        ...
        const pendingFilterClass = activeFilters.pendingDates && activeFilters.pendingDates.length > 0 ? 'filter-active' : '';
        const pendingFilterIcon = activeFilters.pendingDates && activeFilters.pendingDates.length > 0 ? 'filter_alt' : 'filter_list';
...
function renderWATable() {
    ...
    filteredData.forEach(row => {
        ...
        groupedData[key].push({ plate: row.plate, type: row.vehicleType, dates: row.pendingDates });
    });
...
async function exportWAToExcel() {
    ...
    filteredData.forEach(row => {
        ...
        groupedData[key].vehicles.push({ plate: row.plate, type: row.vehicleType, dates: row.pendingDates });
    });

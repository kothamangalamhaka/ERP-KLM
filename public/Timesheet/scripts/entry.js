...
async function triggerFetch() {
    ...
    try {
        const res = await fetch(`/timesheet/api/grid-data?month=${m}&year=${y}&plate=${p}`, { headers: { 'Authorization': 'Bearer ' + token } });
        const data = await res.json();

        const logRes = await fetch(`/timesheet/api/vehicle-logs?plate=${p}`, { headers: { 'Authorization': 'Bearer ' + token } });
        ...
        if (logs.success) {
            ...
            if (activeDrivers.length === 0 && logs.drivers && logs.drivers.length > 0) {
                let sortedDrivers = [...logs.drivers].sort((a, b) => new Date(b.work_start_date || '2000-01-01') - new Date(a.work_start_date || '2000-01-01'));
            ...
            if (activeSites.length === 0 && logs.sites && logs.sites.length > 0) {
                let sortedSites = [...logs.sites].sort((a, b) => new Date(b.work_start_date || '2000-01-01') - new Date(a.work_start_date || '2000-01-01'));
       ...
       if (activeSites.length > 0) {
            ...
            let sStart = latestSiteLog.work_start_date ? latestSiteLog.work_start_date.split('T')[0] : "N/A";
            let sEnd = latestSiteLog.work_end_date ? latestSiteLog.work_end_date.split('T')[0] : "Running";

            document.getElementById('dispSiteStart').innerText = formatDateUI(sStart);
            document.getElementById('dispSiteEnd').innerText = sEnd === "Running" ? "Running" : formatDateUI(sEnd);
            ...
            if (latestSiteLog.status === 'Replaced' && latestSiteLog.replaced_by) {
                ...
                document.getElementById('dispSiteReplace').innerText = latestSiteLog.replaced_by;
            } else {
                document.getElementById('replaceRow').style.display = "none";
            }
        } else {
        ...
        try {
            const monthStr = m + " " + y;
            const invRes = await fetch(`/payment/get-invoice?plate_no=${p}&month=${monthStr}`);
...
function renderGrid(month, year, plate, existingData) {
    ...
    for (let i = 1; i <= days; i++) {
        ...
        if (dbDist !== null && dbDist !== undefined && dbDist !== "") dbDist = parseFloat(dbDist).toFixed(1); else dbDist = "";
...
function updateSummaryBox() {
    ...
    for (let i = 1; i <= days; i++) {
        ...
        let hasLog = false;
        if (bd !== "") {
            if (!isNaN(parseFloat(bd))) { hasLog = true; }
            else { hasLog = false; }
        } else if (ws !== "" && we !== "") {
            hasLog = true;
        }
        ...
        if (bd === 'ID' || bd === 'NP') {
            if (isFullOT) { otHr = 10; } else { normalHr = 10; }
        } else if (bd === 'B' || bd === 'H') {
            // Zero hours
        } else if (tm > 0) {
            if (isFullOT) {
    ...
    inputs.forEach(input => {
        if (input.tagName !== 'TEXTAREA') {
            input.addEventListener('keydown', function (e) {
                const row = parseInt(this.getAttribute('data-row')); const col = this.getAttribute('data-col'); let nextEl = null;
...
function parseRailwayTime(val) {
    if (!val) return 0;
    let [hStr, mStr] = String(val).split('.'); let h = parseInt(hStr) || 0; let m = 0;
...
function calculateRow(rowIdx) {
    ...
    let dist = ""; if (!isNaN(hs) && !isNaN(he)) dist = (he - hs).toFixed(1);
    ...
    const bd = document.querySelector(`.grid-input[data-row="${rowIdx}"][data-col="bd"]`).value.trim().toUpperCase();
...
async function saveCellData(rowIdx, colName, colValue) {
    ...
    try {
        ...
        statusLabel.innerText = '✓ Saved'; statusLabel.className = 'save-indicator status-saved';
...
async function saveInvoiceData() {
    ...
    if (!invoice_no || !bill_nr) {
        await customAlert("Invoice Number and Bill N.Hr are mandatory!", "Validation Error");
        return;
    }
    ...
    if (isEditingInvoice) {
        ...
        if (code !== "imissu") {
            await customAlert("Incorrect Secret Code. Editing cancelled.", "Access Denied");
            return;
        }
    ...
    try {
        ...
        await customAlert("Failed to save data. Check connection.", "Network Error");
    }
}
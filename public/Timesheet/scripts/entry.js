...
function searchPlate() {
  ...
  sug.innerHTML = ""; // 🟢 Fixed typo here (sugg -> sug)
  currentFocus = -1;
...
function getGapStatus(d, sLogs, dLogs) {
  let sActive = false, sGap = false, isReplaced = false;
  ...
  if (!sActive) { if (isReplaced) return "R"; return sGap ? "SC" : "AB"; }
...
function getDaysInMonth(monthStr, year) { return new Date(year, months.indexOf(monthStr) + 1, 0).getDate(); }
function getDayName(dayNum, monthStr, year) { return ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][new Date(`${monthStr} ${dayNum}, ${year}`).getDay()]; }
...
async function triggerFetch() {
  ...
  if (!p) { await customAlert("Please enter a Plate No.", "Missing Information"); return; }
  ...
  btn.disabled = true; text.innerText = "Wait..."; loader.style.display = "block";
  ...
  try {
    ...
    if (logs.success) {
      ...
      if (activeDrivers.length > 0) {
        dNameArr = [...new Set(activeDrivers.map((d) => d.driver_name))].filter((val) => val && String(val).trim() !== "" && String(val).trim() !== "-");
        dMobArr = [...new Set(activeDrivers.map((d) => d.driver_mobile))].filter((val) => val && String(val).trim() !== "" && String(val).trim() !== "-");
      }
    ...
    if (dNameArr.length === 0 && vObjMaster && vObjMaster.driver_name) dNameArr.push(vObjMaster.driver_name);
    if (dMobArr.length === 0 && vObjMaster && vObjMaster.driver_mobile) dMobArr.push(vObjMaster.driver_mobile);
    if (siteArr.length === 0 && vObjMaster && vObjMaster.site_name) siteArr.push(vObjMaster.site_name);

    document.getElementById("dispDName").innerText = dNameArr.length > 0 ? dNameArr.join(" & ") : "N/A";
    document.getElementById("dispDMob").innerText = dMobArr.length > 0 ? dMobArr.join(" & ") : "N/A";
    document.getElementById("dispSite").innerText = siteArr.length > 0 ? siteArr.join(" & ") : "N/A";
    ...
    if (activeSites.length > 0) {
      activeSites.sort((a, b) => new Date(b.work_start_date || "2000-01-01") - new Date(a.work_start_date || "2000-01-01"));
      ...
      if (sEndVal) {
        let endStatusText = latestSiteLog.new_vehicle_no ? " (Replaced)" : " (Released)";
        document.getElementById("dispSiteEnd").innerText = formatDateUI(sEndVal) + endStatusText;
      } else { document.getElementById("dispSiteEnd").innerText = "Running"; }

      document.getElementById("dispAsset").innerText = latestSiteLog.asset_code || (vObjMaster ? vObjMaster.asset_code : null) || "N/A";
      document.getElementById("dispWorkOrder").innerText = latestSiteLog.work_order_no || (vObjMaster ? vObjMaster.wrk_order_no : null) || "N/A";

      if (latestSiteLog.old_vehicle_no && latestSiteLog.old_vehicle_no.trim() !== "") {
        if (document.getElementById("oldVehRow")) { document.getElementById("oldVehRow").style.display = "flex"; document.getElementById("dispOldVeh").innerText = latestSiteLog.old_vehicle_no; }
      } else { if (document.getElementById("oldVehRow")) document.getElementById("oldVehRow").style.display = "none"; }

      if (latestSiteLog.new_vehicle_no && latestSiteLog.new_vehicle_no.trim() !== "") {
        if (document.getElementById("newVehRow")) { document.getElementById("newVehRow").style.display = "flex"; document.getElementById("dispNewVeh").innerText = latestSiteLog.new_vehicle_no; }
      } else { if (document.getElementById("newVehRow")) document.getElementById("newVehRow").style.display = "none"; }
    } else {
      document.getElementById("dispSiteStart").innerText = "N/A"; document.getElementById("dispSiteEnd").innerText = "N/A";
      if (document.getElementById("oldVehRow")) document.getElementById("oldVehRow").style.display = "none";
      if (document.getElementById("newVehRow")) document.getElementById("newVehRow").style.display = "none";
      document.getElementById("dispFieldCo").innerText = vObjMaster ? vObjMaster.field_co || "N/A" : "N/A";
      document.getElementById("dispSiteCo").innerText = vObjMaster ? vObjMaster.site_co || "N/A" : "N/A";
      document.getElementById("dispAsset").innerText = vObjMaster ? vObjMaster.asset_code || "N/A" : "N/A";
      document.getElementById("dispWorkOrder").innerText = vObjMaster ? vObjMaster.wrk_order_no || "N/A" : "N/A";
    }
    ...
    if (siteArr.length > 0) {
      siteArr.forEach((siteName) => {
        let opt = document.createElement("option"); opt.value = siteName; opt.text = siteName; invSiteSelect.appendChild(opt);
      });
      loadInvoiceForSelectedSite();
    } else {
      invSiteSelect.innerHTML = '<option value="">No Site Active</option>'; clearInvoiceForm();
    }

    let existingData = data.success ? data.data : [];
    setTimeout(() => {
      renderGrid(m, y, p, existingData, sStartVal, sEndVal, logs);
      btn.disabled = false; text.innerText = "Fetch Data"; loader.style.display = "none";
    }, 300);
  } catch (error) {
...
function loadInvoiceForSelectedSite() {
  ...
  if (!selectedSite) { clearInvoiceForm(); return; }
  ...
  } else { isEditingInvoice = false; clearInvoiceForm(); }
}

function clearInvoiceForm() {
  document.getElementById("invNo").value = ""; document.getElementById("invBillNo").value = "";
  document.getElementById("invNr").value = ""; document.getElementById("invOt").value = "";
  document.getElementById("invAmt").value = ""; isEditingInvoice = false;
}

function renderGrid(month, year, plate, existingData, siteStart, siteEnd, logs = { drivers: [], sites: [] }) {
  const tbody = document.getElementById("gridBody"); tbody.innerHTML = "";
  const siteLogs = logs.sites || []; const driverLogs = logs.drivers || [];
  const days = getDaysInMonth(month, year); const mIdx = months.indexOf(month);

  const cleanVal = (val) => val === null || val === "null" || val === undefined ? "" : val;

  for (let i = 1; i <= days; i++) {
    const rowData = existingData.find((r) => parseInt(r.record_date) === i) || {};
    let dbDist = cleanVal(rowData.calc_distance); if (dbDist !== "") dbDist = parseFloat(dbDist).toFixed(1);

    let dayName = getDayName(i, month, year);
    let rowClass = dayName === "Fri" ? "row-friday" : "";
    let currentDateObj = new Date(year, mIdx, i); currentDateObj.setHours(0, 0, 0, 0);
    ...
    let specialRule = specialRulesCache.find((r) => r.is_active && (r.sites.includes("ALL") || r.sites.includes(currentSiteStr)) && r.dates.includes(formattedDate));
    ...
    if (!hasData) {
      if (specialRule && specialRule.rule_type !== "FULL_OT") { displayBd = specialRule.rule_type; rowRemark = specialRule.reason || ""; } 
      else if (statusCode !== "ACTIVE") { displayBd = statusCode; }
    }
  ...
  attachGridEvents(); updateSummaryBox();
}

function updateSummaryBox() {
  let tNormal = 0, tOT = 0, tTime = 0, tDist = 0, tFuel = 0, logCount = 0;
  ...
  for (let i = 1; i <= days; i++) {
    ...
    let fl = parseFloat(document.querySelector(`.grid-input[data-row="${i}"][data-col="fuel"]`)?.value) || 0;
    let bd = document.querySelector(`.grid-input[data-row="${i}"][data-col="bd"]`)?.value.trim().toUpperCase() || "";
    let ws = document.querySelector(`.grid-input[data-row="${i}"][data-col="wrk_start"]`)?.value.trim() || "";
    let we = document.querySelector(`.grid-input[data-row="${i}"][data-col="wrk_end"]`)?.value.trim() || "";

    let hasLog = (bd !== "" && !isNaN(parseFloat(bd))) || (ws !== "" && we !== "");
    if (hasLog) logCount++;
    ...
    let sumOtRule = specialRulesCache.find((r) => r.is_active && r.rule_type === "FULL_OT" && (r.sites.includes("ALL") || r.sites.includes(siteForSum)) && r.dates.includes(formattedDateForSum));
    ...
    tNormal += normalHr; tOT += otHr; tDist += dt; tFuel += fl;
  }
  tTime = tNormal + tOT;
  document.getElementById("logSheetCount").innerText = logCount > 0 ? `( ${logCount} )` : "";
  ...
  inputs.forEach((input) => {
    if (input.tagName !== "TEXTAREA") {
      input.addEventListener("keydown", function (e) {
        ...
        let nextEl = null;
        if (e.key === "ArrowDown") nextEl = document.querySelector(`.grid-input[data-row="${row + 1}"][data-col="${col}"]`);
        else if (e.key === "ArrowUp") nextEl = document.querySelector(`.grid-input[data-row="${row - 1}"][data-col="${col}"]`);
        else if (e.key === "ArrowRight" && this.selectionStart === this.value.length) {
          e.preventDefault(); let td = this.closest("td").nextElementSibling;
          while (td && !td.querySelector(".grid-input")) td = td.nextElementSibling;
          if (td) nextEl = td.querySelector(".grid-input");
        } else if (e.key === "ArrowLeft" && this.selectionStart === 0) {
          e.preventDefault(); let td = this.closest("td").previousElementSibling;
          while (td && !td.querySelector(".grid-input")) td = td.previousElementSibling;
          if (td) nextEl = td.querySelector(".grid-input");
        }
    ...
    input.addEventListener("blur", function () {
      ...
      calculateRow(row); saveCellData(row, col, val); updateSummaryBox();
    });
...
function parseRailwayTime(val) {
  ...
  let h = parseInt(hStr) || 0; let m = 0;
  if (mStr) { mStr = mStr.length === 1 ? mStr + "0" : mStr.substring(0, 2); m = parseInt(mStr); }
  return h + m / 60;
}

function customRound(val) {
  let h = Math.floor(val); let m = Math.round((val - h) * 60); return m >= 45 ? h + 1 : h;
}

function calculateRow(rowIdx) {
  const hs = parseFloat(document.querySelector(`.grid-input[data-row="${rowIdx}"][data-col="hmr_start"]`).value);
  const he = parseFloat(document.querySelector(`.grid-input[data-row="${rowIdx}"][data-col="hmr_end"]`).value);
  let dist = "";
  if (!isNaN(hs) && !isNaN(he)) dist = (he - hs).toFixed(1);
  document.getElementById(`dist_${rowIdx}`).value = dist;

  const ws = document.querySelector(`.grid-input[data-row="${rowIdx}"][data-col="wrk_start"]`).value;
  const we = document.querySelector(`.grid-input[data-row="${rowIdx}"][data-col="wrk_end"]`).value;
  let bdInput = document.querySelector(`.grid-input[data-row="${rowIdx}"][data-col="bd"]`);
  ...
  if (bd) {
    ...
    let formattedDateForOT = currentDate.toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }).replace(/ /g, " ");

    let otRule = specialRulesCache.find((r) => r.is_active && r.rule_type === "FULL_OT" && (r.sites.includes("ALL") || r.sites.includes(site)) && r.dates.includes(formattedDateForOT));

    let breakOverlap = 0;
    let activeBreakRule = breakRulesCache.find((r) => {
        if (!r.is_active) return false;
        let sitesArray = [];
        try { sitesArray = typeof r.sites === 'string' ? JSON.parse(r.sites) : r.sites; } catch (e) { sitesArray = []; }
        
        // 🟢 FIXED: Partial Keyword Match
        let siteMatch = sitesArray.includes("ALL") || sitesArray.some(keyword => site.includes(keyword));
        
        if (!siteMatch) return false;
        let ruleStart = new Date(r.start_date); ruleStart.setHours(0,0,0,0);
        let ruleEnd = new Date(r.end_date); ruleEnd.setHours(23,59,59,999);
    ...
    if (nl || !!otRule) {
      ...
      let rule = rulesCache.find((r) => site.includes(r.site_keyword)) || rulesCache.find((r) => r.site_keyword === "DEFAULT");
...
async function saveCellData(rowIdx, colName, colValue) {
  ...
  statusLabel.innerText = "Saving..."; statusLabel.className = "save-indicator status-saving";
  ...
  const payload = { month: document.getElementById("selMonth").value, year: document.getElementById("selYear").value, plate_no: plate, record_date: rowIdx, col_name: colName, col_value: colValue, calc_distance: calc_distance, calc_time: calc_time };

  try {
    await fetch("/timesheet/api/upsert-grid-cell", { method: "POST", headers: { "Content-Type": "application/json", Authorization: "Bearer " + token }, body: JSON.stringify(payload) });

    clearTimeout(saveTimeout); statusLabel.innerText = "✓ Saved"; statusLabel.className = "save-indicator status-saved";
    saveTimeout = setTimeout(() => { statusLabel.className = "save-indicator"; }, 2000);

    try {
      const user = JSON.parse(localStorage.getItem("timesheetUser"));
      const mIdx = months.indexOf(payload.month) + 1;
      const padMonth = String(mIdx).padStart(2, "0"); const padDay = String(rowIdx).padStart(2, "0");
      ...
      if (!loggedRowsTracker.has(logKey)) {
        ...
        await fetch("/api/entrylog/add", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${localStorage.getItem("timesheetToken")}` }, body: JSON.stringify({ username: user ? user.username : "Unknown", plate_number: payload.plate_no, entry_date: formattedDate, action: "UPDATE" }) });
      }
    } catch (logErr) {}
  } catch (e) { statusLabel.innerText = "Error"; statusLabel.style.backgroundColor = "#dc3545"; }
}
...
function customAlert(message, title = "Notice") {
  return new Promise((resolve) => {
    ...
    window.closeCustomAlert = function () { document.getElementById("customAlertModal").style.display = "none"; resolve(); };
  });
}

function customPrompt(message, isPassword = false, title = "Input Required") {
  return new Promise((resolve) => {
    document.getElementById("customPromptTitle").innerText = title; document.getElementById("customPromptMessage").innerText = message;
    const inputEl = document.getElementById("customPromptInput"); inputEl.type = isPassword ? "password" : "text"; inputEl.value = "";
    document.getElementById("customPromptModal").style.display = "flex"; inputEl.focus();
    window.submitCustomPrompt = function () { const val = inputEl.value; document.getElementById("customPromptModal").style.display = "none"; resolve(val); };
    window.closeCustomPrompt = function () { document.getElementById("customPromptModal").style.display = "none"; resolve(null); };
  });
}

async function saveInvoiceData() {
  const plate_no = document.getElementById("selPlate").value.trim().toUpperCase();
  if (!plate_no) { await customAlert("Please fetch a Plate Number first.", "Action Required"); return; }
  const site_name = document.getElementById("invSiteSelect").value;
  if (!site_name) { await customAlert("Please select a Site before saving.", "Action Required"); return; }
  const month = document.getElementById("selMonth").value + " " + document.getElementById("selYear").value;
  const invoice_no = document.getElementById("invNo").value.trim(); const bill_no = document.getElementById("invBillNo").value.trim();
  const bill_nr = document.getElementById("invNr").value.trim(); const bill_ot = document.getElementById("invOt").value.trim();
  const invoice_amount = document.getElementById("invAmt").value.trim();

  if (!invoice_no || !bill_nr) { await customAlert("Invoice Number and Bill N.Hr are mandatory!", "Validation Error"); return; }

  let edit_reason = "";
  if (isEditingInvoice) {
    let code = await customPrompt("Enter Secret Code to edit existing record:", true, "Security Check");
    if (code !== "imissu") { await customAlert("Incorrect Secret Code. Editing cancelled.", "Access Denied"); return; }
    edit_reason = await customPrompt("Please enter the reason for editing:", false, "Edit Reason");
    if (!edit_reason || edit_reason.trim() === "") { await customAlert("Edit reason is required!", "Validation Error"); return; }
  }

  const payload = { plate_no, month, site_name, invoice_no, bill_no, bill_nr, bill_ot, invoice_amount, edit_reason };
  try {
    const res = await fetch("/payment/save-invoice", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    const data = await res.json();
    if (data.success) {
      await customAlert(data.message, "Success"); isEditingInvoice = true;
      let existingIdx = currentInvoices.findIndex((i) => i.site_name === site_name);
      if (existingIdx > -1) currentInvoices[existingIdx] = payload; else currentInvoices.push(payload);
    } else { await customAlert("Error: " + data.message, "Error"); }
  } catch (e) { await customAlert("Failed to save data. Check connection.", "Network Error"); }
}

function savePlateHistory(plate) {
  if (!plate) return;
  let history = JSON.parse(localStorage.getItem("plateSearchHistory") || "[]");
  history = history.filter((p) => p !== plate); history.unshift(plate);
  if (history.length > 30) history = history.slice(0, 30);
  localStorage.setItem("plateSearchHistory", JSON.stringify(history));
}

function toggleUserMenu(e) { e.stopPropagation(); const menu = document.getElementById("userDropdownMenu"); menu.style.display = menu.style.display === "flex" ? "none" : "flex"; }
document.addEventListener("click", function (e) { if (!e.target.closest(".user-profile-container")) { const menu = document.getElementById("userDropdownMenu"); if (menu) menu.style.display = "none"; } });
function toggleDarkMode() { const isDark = document.body.classList.toggle("dark-mode"); localStorage.setItem("timesheetTheme", isDark ? "dark" : "light"); document.getElementById("userDropdownMenu").style.display = "none"; }
function logout() { localStorage.removeItem("timesheetToken"); localStorage.removeItem("timesheetUser"); window.location.href = "index.html"; }
...
async function exportExcel() {
  ...
  try {
    const res = await fetch(`/timesheet/api/grid-data?month=${m}&year=${y}`, { headers: { Authorization: "Bearer " + token } });
    const data = await res.json();
    let ws_data = [["Plate No", "Date", "Wrk Start", "Wrk end", "HMR Start", "HMR End", "Fuel", "BD", "Remark", "NL", "Distance", "Time"]];
    if (data.success && data.data && data.data.length > 0) {
      data.data.forEach((row) => {
        ws_data.push([row.plate_no, row.record_date, row.wrk_start || "", row.wrk_end || "", row.hmr_start || "", row.hmr_end || "", row.fuel || "", row.bd || "", row.remark || "", row.nl_checked ? "TRUE" : "FALSE", row.calc_distance || "", row.calc_time || ""]);
      });
    } else {
      ws_data.push(["EXAMPLE-123", "1", "6.00", "18.00", "1000", "1150", "150", "", "Sample Entry", "FALSE", "", ""]);
      await customAlert("Notice", "No data found for this month. Exporting blank template.");
    }
    var ws = XLSX.utils.aoa_to_sheet(ws_data); var wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Timesheet");
    var wbout = XLSX.write(wb, { bookType: "xlsx", type: "binary" });
    let blob = new Blob([s2ab(wbout)], { type: "application/octet-stream" });
    let url = window.URL.createObjectURL(blob); let a = document.createElement("a"); document.body.appendChild(a); a.href = url; a.download = `Timesheet_DailyData_${m}_${y}.xlsx`; a.click(); document.body.removeChild(a);
  } catch (error) { customAlert("Error", "Failed to export data. Check server connection."); }
}
...
async function importExcel() {
  ...
  if (!file) { customAlert("Warning", "Please select an Excel file first."); return; }
  const m = document.getElementById("bulkMonth").value;
  const y = document.getElementById("bulkYear").value;
  const sts = document.getElementById("importStatus");
  sts.innerText = "Parsing Excel... Please wait."; sts.style.color = "#ffc107"; sts.style.backgroundColor = "#fff3cd"; sts.style.border = "1px solid #ffe69c";

  const token = localStorage.getItem("timesheetToken");
  let rules = []; let spRules = []; let bkRules = [];
  try {
    const rRes = await fetch("/timesheet/api/rules", { headers: { Authorization: "Bearer " + token } });
    const rData = await rRes.json(); if (rData.success) rules = rData.data;

    let vInfo = [];
    const vRes = await fetch("/timesheet/api/vehicle-info", { headers: { Authorization: "Bearer " + token } });
    const vData = await vRes.json(); if (vData.success) vInfo = vData.data;

    // 🟢 FETCHING CACHES LOCALLY IN IMPORT FOR ACCURACY
    const srRes = await fetch("/timesheet/api/special-rules", { headers: { Authorization: "Bearer " + token } });
    const srData = await srRes.json(); if (srData.success) spRules = srData.data;

    const brRes = await fetch("/timesheet/api/break-rules", { headers: { Authorization: "Bearer " + token } });
    const brData = await brRes.json(); if (brData.success) bkRules = brData.data;

    function calcRowDistTime(row, site, recordDate) {
      let hs = parseFloat(row["HMR Start"]); let he = parseFloat(row["HMR End"]); let dist = null;
      if (!isNaN(hs) && !isNaN(he)) dist = (he - hs).toFixed(2);
      let finalTime = null; let ws = String(row["Wrk Start"] || "").trim(); let we = String(row["Wrk end"] || "").trim(); let bd = String(row["BD"] || "").trim().toUpperCase(); let nlRaw = String(row["NL"]).trim().toUpperCase(); let nl = nlRaw === "TRUE" || nlRaw === "Y" || nlRaw === "1";
      ...
      if (bd) {
        ...
        let cRound = (val) => { let h = Math.floor(val); let mm = Math.round((val - h) * 60); return mm >= 45 ? h + 1 : h; };

        let endIsMorning = eHour >= 6 && eHour <= 12.5; let isNightShift = sHour >= 15 || endIsMorning;
        let currentDate = new Date(y, months.indexOf(m), parseInt(recordDate));
        let formattedDateForOT = currentDate.toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }).replace(/ /g, " ");

        let otRule = spRules.find((r) => r.is_active && r.rule_type === "FULL_OT" && (r.sites.includes("ALL") || r.sites.includes(site)) && r.dates.includes(formattedDateForOT));
        ...
        if (activeBreakRule && !isNightShift) {
            let bStart = parseRT(activeBreakRule.break_start); let bEnd = parseRT(activeBreakRule.break_end);
            let overlapStart = Math.max(sHour, bStart); let overlapEnd = Math.min(eHour, bEnd);
            if (overlapStart < overlapEnd) breakOverlap = overlapEnd - overlapStart;
        }

        if (nl || !!otRule) { finalTime = cRound(diff); } else if (activeBreakRule && !isNightShift) { finalTime = cRound(diff - breakOverlap); } else if (isNightShift) { finalTime = cRound(diff); } else {
          let rule = rules.find((r) => site.includes(r.site_keyword)) || rules.find((r) => r.site_keyword === "DEFAULT");
          let ded = rule ? rule.default_deduct : 1;
          if (rule && diff <= 11) ded = rule.deduct_under_11; else if (rule && diff >= 12) ded = rule.deduct_over_12;
    ...
    reader.onload = async function (e) {
      const data = new Uint8Array(e.target.result); const workbook = XLSX.read(data, { type: "array" }); const firstSheet = workbook.Sheets[workbook.SheetNames[0]];
      const jsonData = XLSX.utils.sheet_to_json(firstSheet, { defval: "", raw: false });
      let processedRecords = [];
      jsonData.forEach((row) => {
        let pNo = String(row["Plate No"]).trim(); let rDateRaw = String(row["Date"]).trim(); let rDate = parseInt(rDateRaw);
        if (!pNo || isNaN(rDate) || rDate < 1 || rDate > 31 || pNo.includes("EXAMPLE")) return;
        let vehicle = vInfo.find((v) => v.plate_no === pNo); let site = vehicle ? (vehicle.site_name || "").toUpperCase() : "";
        let calcRes = calcRowDistTime(row, site, rDate);
        let nlRaw = String(row["NL"]).trim().toUpperCase(); let isNlChecked = nlRaw === "TRUE" || nlRaw === "Y" || nlRaw === "1";
        processedRecords.push({ month: m, year: y, plate_no: pNo, record_date: rDate.toString(), wrk_start: String(row["Wrk Start"]).trim() || null, wrk_end: String(row["Wrk end"]).trim() || null, hmr_start: String(row["HMR Start"]).trim() || null, hmr_end: String(row["HMR End"]).trim() || null, fuel: String(row["Fuel"]).trim() || null, bd: String(row["BD"]).trim() || null, remark: String(row["Remark"]).trim() || "", nl_checked: isNlChecked, calc_distance: calcRes.dist, calc_time: calcRes.finalTime });
      });

      if (processedRecords.length === 0) { sts.innerText = "No valid data found in Excel."; sts.style.color = "#842029"; sts.style.backgroundColor = "#f8d7da"; return; }
      sts.innerText = `Saving ${processedRecords.length} exact records...`;
      try {
        const sendRes = await fetch("/timesheet/api/bulk-import", { method: "POST", headers: { "Content-Type": "application/json", Authorization: "Bearer " + token }, body: JSON.stringify({ records: processedRecords }) });
        const resData = await sendRes.json();
        if (sendRes.ok && resData.success) { sts.innerText = "✓ Import Successful! Zero Errors."; sts.style.color = "#0f5132"; sts.style.backgroundColor = "#d1e7dd"; document.getElementById("excelFile").value = ""; } else { sts.innerText = "Error: " + (resData.message || "Upload Failed"); sts.style.color = "#842029"; sts.style.backgroundColor = "#f8d7da"; }
      } catch (err) { sts.innerText = "Network Error: Check Server connection."; sts.style.color = "#842029"; sts.style.backgroundColor = "#f8d7da"; }
    };
    reader.readAsArrayBuffer(file);
  } catch (err) { sts.innerText = "Error initializing process. Try again."; sts.style.color = "#842029"; sts.style.backgroundColor = "#f8d7da"; }
}

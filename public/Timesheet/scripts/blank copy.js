...
// Filtering Variables
let activeFilters = { owner: [], driver: [], site: [], plate: [] };
...
document.addEventListener('click', function (e) {
    ...
    if (!e.target.closest('.excel-filter-menu') && !e.target.classList.contains('filter-icon')) {
        closeCustomFilter();
    }
});
...
function openFilterMenu(event, key, colIndex) {
    ...
    for (let i = 0; i < trs.length; i++) {
        ...
        for (let filterKey in activeFilters) {
            ...
            let cIdx = filterKey === 'owner' ? 0 : filterKey === 'driver' ? 2 : filterKey === 'site' ? 4 : 5;
...
document.getElementById('filterSearchInput')?.addEventListener('keyup', function () {
    ...
    document.querySelectorAll('.filter-row-item').forEach(item => {
        if (item.textContent.toLowerCase().includes(keyword)) item.style.display = '';
...
function applyCustomFilter() {
    ...
    const icon = document.querySelector(`th span.filter-icon[onclick*="'${key}'"]`);

    if (checkedCount === totalVisible || checkedCount === 0) {
        activeFilters[key] = [];
        if (icon) { icon.classList.remove('filter-active'); icon.innerText = 'filter_list'; }
    } else {
        activeFilters[key] = selectedValues;
        if (icon) { icon.classList.add('filter-active'); icon.innerText = 'filter_alt'; }
    }
...
function applyGridFilters() {
    const globalSearch = document.getElementById('globalSearch').value.toUpperCase();
    ...
    for (let i = 0; i < trs.length; i++) {
        ...
        let tOwner = tds[0].textContent.trim();
        let tDriver = tds[2].textContent.trim();
        let tSite = tds[4].textContent.trim();
        let tPlate = tds[5].textContent.trim();
        let rowText = row.textContent.toUpperCase();

        let matchesFilters = true;

        if (activeFilters.owner.length > 0 && !activeFilters.owner.includes(tOwner)) matchesFilters = false;
        if (activeFilters.driver.length > 0 && !activeFilters.driver.includes(tDriver)) matchesFilters = false;
        if (activeFilters.site.length > 0 && !activeFilters.site.includes(tSite)) matchesFilters = false;
        if (activeFilters.plate.length > 0 && !activeFilters.plate.includes(tPlate)) matchesFilters = false;
...
async function generatePendingReport() {
    ...
    container.innerHTML = '<div style="color: #64748b; margin-top: 50px; text-align:center;">Analyzing blank cells...</div>';

    try {
        // Reset filters on new fetch
        activeFilters = { owner: [], driver: [], site: [], plate: [] };
        ...
        data.vehicles.forEach((v) => {
            ...
            let driverMob = activeDrivers.map(d => d.driver_mobile).join(' & ') || (v.driver_mobile || "");
            ...
            if (blankDays.length > 0) {
                pendingList.push({ owner: ownerName, ownerMob: ownerMob, driver: currDriver, driverMob: driverMob, site: currSite, plate: plate, pendingDates: blankDays.join(', ') });
            }
        });
        ...
        if (pendingList.length === 0) {
            container.innerHTML = '<div style="color: #10b981; margin-top: 50px; text-align:center; font-weight:bold; font-size: 18px;">✅ All clear! No pending log sheets found up to the selected date.</div>';
            return;
        }

        // 🟢 Global Sort: Owner Name -> Site Name
        pendingList.sort((a, b) => {
            let ownerA = a.owner || ""; let ownerB = b.owner || "";
            let siteA = a.site || ""; let siteB = b.site || "";
            let ownerCmp = ownerA.localeCompare(ownerB);
            if (ownerCmp !== 0) return ownerCmp;
            return siteA.localeCompare(siteB);
        });

        const ownerFilterClass = activeFilters.owner && activeFilters.owner.length > 0 ? 'filter-active' : '';
        const ownerFilterIcon = activeFilters.owner && activeFilters.owner.length > 0 ? 'filter_alt' : 'filter_list';

        const driverFilterClass = activeFilters.driver && activeFilters.driver.length > 0 ? 'filter-active' : '';
        const driverFilterIcon = activeFilters.driver && activeFilters.driver.length > 0 ? 'filter_alt' : 'filter_list';

        const siteFilterClass = activeFilters.site && activeFilters.site.length > 0 ? 'filter-active' : '';
        const siteFilterIcon = activeFilters.site && activeFilters.site.length > 0 ? 'filter_alt' : 'filter_list';

        const plateFilterClass = activeFilters.plate && activeFilters.plate.length > 0 ? 'filter-active' : '';
        const plateFilterIcon = activeFilters.plate && activeFilters.plate.length > 0 ? 'filter_alt' : 'filter_list';
        ...
        pendingList.forEach(row => {
            ...
            <tr>
                ...
                <td class="plate-col">${row.plate}</td>
        ...
        <table class="ts-table" id="pendingTable">
            <colgroup id="dbColGroup">
                ...
                <col class="col-hide-export" style="width: 15%;">
                <col class="col-hide-export" style="width: 8%;">
                <col style="width: 10%;">
                <col style="width: 10%;">
                <col style="width: 34%;">
                ...
                <tr>
                    <th>
                        <div class="col-header-wrap">
                            Owner Name
                            <span class="material-icons filter-icon ${ownerFilterClass}" onclick="openFilterMenu(event, 'owner', 0)">${ownerFilterIcon}</span>
                        </div>
                    ...
                    <th class="col-hide-export">
                        <div class="col-header-wrap">
                            Driver Name
                            <span class="material-icons filter-icon ${driverFilterClass}" onclick="openFilterMenu(event, 'driver', 2)">${driverFilterIcon}</span>
                        </div>
                        <div class="resizer"></div>
                    </th>
                    <th class="col-hide-export">
                    ...
                    <th>
                        <div class="col-header-wrap">
                            Site Name
                            <span class="material-icons filter-icon ${siteFilterClass}" onclick="openFilterMenu(event, 'site', 4)">${siteFilterIcon}</span>
                        </div>
                        <div class="resizer"></div>
                    </th>
                    <th>
                        <div class="col-header-wrap">
                            Plate No
                            <span class="material-icons filter-icon ${plateFilterClass}" onclick="openFilterMenu(event, 'plate', 5)">${plateFilterIcon}</span>
                        </div>
                        <div class="resizer"></div>
                    </th>
                    <th>
                        <div class="col-header-wrap">Pending Dates (${m} ${y})</div>
                        <div class="resizer"></div>
                    </th>
                </tr>
            </thead>
...
function closeWAModal() {
    ...
    document.getElementById('waBody').innerHTML = '<div style="text-align: center; color: #64748b; font-size: 12px; margin-top:10px;">Please fetch data and make a selection to view records.</div>';
...
function toggleWAFilter() {
    ...
    if (type === 'Site Name') {
        ...
        uniqueOptions = [...new Set(globalPendingData.map(item => item.site))].filter(Boolean).sort();
    } else {
        labelEl.innerText = "Select Owner Name(s)";
        uniqueOptions = [...new Set(globalPendingData.map(item => item.owner))].filter(Boolean).sort();
    }
...
function checkWASelectAll() {
    ...
    document.getElementById('waSelectAll').checked = (total > 0 && total === checked);
}
...
function renderWATable() {
    ...
    if (selectedValues.length === 0) {
        waBody.innerHTML = '<div style="text-align: center; color: #64748b; font-size: 12px; margin-top:10px;">Please select at least one option to view records.</div>';
        return;
    }
    ...
    if (filteredData.length === 0) {
        waBody.innerHTML = '<div style="text-align: center; color: #dc3545; font-size: 12px; margin-top:10px; font-weight:600;">No pending data for this selection.</div>';
        return;
    }
    ...
    filteredData.forEach(row => {
        let mobileNo = isSiteMode ? (row.driverMob || "No Mobile") : (row.ownerMob || "No Mobile");
        let key = mobileNo.trim();

        if (!groupedData[key]) { groupedData[key] = []; }

        groupedData[key].push({ plate: row.plate, dates: row.pendingDates });
    });
    ...
    for (const [mobile, vehicles] of Object.entries(groupedData)) {
        ...
        vehicles.forEach((v, index) => {
            ...
            infoBlock += `<span style="font-weight:700; color:#0f2027;">${v.plate}</span><br><span style="color:#4b5563;">${formattedDates}</span>`;
            if (index < vehicles.length - 1) { infoBlock += `<br><br>`; }
        });
...
async function copyWAToClipboard() {
    ...
    if (!table) { await customAlert("Warning", 'No data to copy. Please make a selection first.'); return; }
    ...
    try {
        ...
        await customAlert("Success", 'Data copied to clipboard! You can now paste it into WhatsApp or Excel.');
    } catch (err) {
...
async function exportWAToExcel() {
    ...
    filteredData.forEach(row => {
        ...
        groupedData[key].vehicles.push({ plate: row.plate, dates: row.pendingDates });
    });
    ...
    for (const [mobile, data] of Object.entries(groupedData)) {
        ...
        data.vehicles.forEach((v, index) => {
            ...
            infoBlock += `${v.plate}\n${formattedDates}`;
            if (index < data.vehicles.length - 1) { infoBlock += `\n\n`; }
        });
    ...
    if (isSiteMode) {
        ...
        ws['!cols'] = [{ wch: 30 }, { wch: 20 }, { wch: 30 }, { wch: 25 }];
    }
...
function toggleLoad(show) {
    document.getElementById('exportLoader').style.display = show ? 'inline-block' : 'none';
...
function getFilteredDataForExport() {
    ...
    for (let i = 0; i < trs.length; i++) {
        if (trs[i].style.display !== 'none') {
            data.push({
                owner: trs[i].cells[0].innerText.trim(),
                ownerMob: trs[i].cells[1].innerText.trim(),
                driver: trs[i].cells[2].innerText.trim(),
                driverMob: trs[i].cells[3].innerText.trim(),
                site: trs[i].cells[4].innerText.trim(),
                plate: trs[i].cells[5].innerText.trim(),
                pendingDates: trs[i].cells[6].innerText.trim()
            });
        }
    }
    return data;
...
async function exportToExcel() {
    ...
    if (visibleData.length === 0) { await customAlert("Warning", "No visible data to export."); return; }
    ...
    let sheetData = [["Driver Mobile", "Pending Dates", "Site Name", "Owner Name", "Owner Mobile"]];
...
async function exportToPDF() {
    ...
    if (data.length === 0) { await customAlert("Warning", "No visible data to export."); return; }
    ...
    <table style="width: 100%; border-collapse: collapse; font-size: 11px; table-layout: fixed; font-family: sans-serif;">
        <thead>
            <tr>
                ...
                <th style="border: 1px solid #000; padding: 10px; background-color: #e67e22; color: #fff; text-align: left; width: 10%;">Site Name</th>
                <th style="border: 1px solid #000; padding: 10px; background-color: #e67e22; color: #fff; text-align: center; width: 10%;">Plate No</th>
                <th style="border: 1px solid #000; padding: 10px; background-color: #e67e22; color: #fff; text-align: left; width: 34%;">Pending Dates</th>
            </tr>
        </thead>
        <tbody>
`;
    ...
    html2pdf().set(opt).from(container).save().then(() => toggleLoad(false));
}
...
async function createA4Images(groupName, dataSubset, m, y) {
    ...
    for (let i = 0; i < dataSubset.length; i += ROWS_PER_PAGE) {
        ...
        let title = groupName === 'All' ? `Pending Logs - ${m} ${y}` : `${groupName} - Pending Logs (${m} ${y})`;
    ...
    `;
        ...
        let fileName = groupName === 'All' ? `Pending_Logs_Page_${pageNum}.png` : `${groupName}_Page_${pageNum}.png`;
    ...
    try {
        ...
        if (mode === 'All') {
            ...
            data.forEach(row => {
                ...
                const key = `${row.owner}_${siteFirstWord}`.replace(/[^a-zA-Z0-9_]/g, '_');

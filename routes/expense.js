...
router.post('/login', (req, res) => {
    ...
    if (!validUserId || !validPassword) {
        return res.status(500).json({ success: false, message: 'Server credentials not configured properly.' });
    }

    if (userid === validUserId && password === validPassword) {
        const token = jwt.sign({ role: 'expense_admin' }, JWT_SECRET, { expiresIn: '12h' });
...
router.get('/', async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM journal_entries ORDER BY entry_date ASC, id ASC');
...
router.delete('/delete/:id', async (req, res) => {
    try {
        ...
        if (!validKey) {
            return res.status(500).json({ success: false, message: 'Delete key not configured on server' });
        }

        if (key !== validKey) {
            return res.status(401).json({ success: false, message: 'Invalid Delete Security Key' });
        }
...
router.get('/export-excel', async (req, res) => {
    try {
        ...
        res.setHeader('Content-Disposition', 'attachment; filename=KLM_Expense_Report.xlsx');
